...
def get_articles(request):
    ...
    for topick in topicskeywords:
        topicobjs = Topic.objects.filter(
            keyword=topick).order_by('-probability')[:10]
        topics.update([tobj.topic for tobj in topicobjs])
    # print(topics)
    for t in topics:
        articletopics = ArticleTopic.objects.filter(
            topicId=t).order_by('-probability')[:10]
        articletopics_set.update(articletopics)
    # print(articletopics_set)
    res.append([{'text': article.articleId.article, 'url': article.articleId.url,
                 'title': article.articleId.title, 'summary': article.articleId.summary} for article in articletopics_set])
    # print(res)
